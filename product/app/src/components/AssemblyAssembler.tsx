        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {currentStep === 'review' && (
            <div className="space-y-6">
              ...
              {/* Manual Add Item - available for ALL assembly types */}
              <div className="pt-4 border-t border-slate-700">
                ...
                {showManualSearch && (
                  <div className="mt-3 space-y-2">
                      ...
                      <div className="max-h-48 overflow-y-auto space-y-1 bg-slate-700/50 rounded-lg p-2">
                        {manualSearchResults.map((item) => (
                          <button
                            key={item.taskCode}
                            onClick={() => {
                              const factor = prompt('Quantity factor (1.0 = 1:1, 0.1 = 1 per 10 LF):', '1.0');
                              const qtyFactor = factor ? parseFloat(factor) : 1.0;
                              if (!isNaN(qtyFactor)) {
                                setCustomItems(prev => [...prev, { 
                                  jocItem: item, 
                    ...
                    {manualSearchQuery.length >= 2 && manualSearchResults.length === 0 && (
                      <div className="text-slate-400 text-sm p-2">No items found</div>
                    )}
                  </div>
                )}

                {/* Show custom items with delete option */}
                {customItems.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {customItems.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-3 p-2 bg-purple-500/10 border border-purple-500/30 rounded-lg">
                        <div className="flex-1 min-w-0">
                          <div className="text-white text-sm truncate">{item.jocItem.description}</div>
                          <div className="text-xs text-slate-400">
                            ${item.jocItem.unitCost.toFixed(2)}/{item.jocItem.unit} × {item.quantityFactor}
                          </div>
                        </div>
                        <button
                          onClick={() => setCustomItems(prev => prev.filter((_, i) => i !== idx))}
